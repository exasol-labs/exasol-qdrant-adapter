...
The adapter SHALL intercept `INSERT INTO` statements targeting a vector virtual schema table and forward each row's text value to Qdrant as a point upsert. Qdrant SHALL compute and store the embedding using the collection's configured inference model. The original text SHALL be stored as a Qdrant payload field named `text`, and the source ID SHALL be stored as a payload field named `_original_id`. These payload field names are shared with the UDF ingestion path so that search results are consistent regardless of ingestion method.

#### Scenario: Single row insert
- **WHEN** a user executes `INSERT INTO <schema>.<table> VALUES ('<id>', '<text>')`
- **THEN** the adapter upserts a point to the corresponding Qdrant collection with a UUID derived from `<id>`, the original `<id>` stored in payload field `_original_id`, the `<text>` stored in payload field `text`, and the embedding computed by Qdrant's inference API
