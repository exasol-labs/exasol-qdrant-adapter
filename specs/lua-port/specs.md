...
**Decision:** The Java adapter is being **replaced** by the Lua adapter. We accept the TLS limitation. Users who require self-signed or private CA TLS will need to maintain their own Java adapter fork. Downsides are documented in `docs/lua-port/limitations.md`.
...
- **Delete** `src/main/java/` and `pom.xml` — the Java adapter is fully replaced
